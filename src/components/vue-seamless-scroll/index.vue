<template>
  <!--
    html==
    <vueSeamless v-if="list.length>0" :data="list" :class-option="scrollOption">
   ...
   -->
  <div ref="wrap">
    <div :style="leftSwitch" v-if="navigation" :class="leftSwitchClass" @click="leftSwitchClick"><slot name="left-switch"></slot></div>
    <div :style="rightSwitch" v-if="navigation" :class="rightSwitchClass" @click="rightSwitchClick"><slot name="right-switch"></slot></div>
   <!-- <div
      ref="realBox"
      :style="pos"
      @mouseenter="enter"
    ...
    <div
      ...
      @mousedown="touchStart"
    >
      <div ref="slotList" :style="float"><slot></slot></div>
...
export default {
  ...
  methods: {
    ...
    touchMove(e) {
      if (!this.canTouchScroll || (e.scale && e.scale !== 1)) return;
      let touch;
      if (e.type == 'touchmove') {
        //移动端
        touch = e.targetTouches[0];
